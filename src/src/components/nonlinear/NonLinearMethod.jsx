...
export default function NonLinearMethod({ methodId, methodName }) {
  ...
  const [activeField, setActiveField] = useState('expr');

  const insertText = (text) => {
    setInputs(prev => ({ ...prev, [activeField]: (prev[activeField] || '') + text }));
  };
  const backspace = () => {
    setInputs(prev => ({ ...prev, [activeField]: (prev[activeField] || '').slice(0, -1) }));
  };
  const clearField = () => {
    setInputs(prev => ({ ...prev, [activeField]: '' }));
  };
  ...
  const chartXMin = needsInterval ? parseFloat(inputs.a) - 1 : (parseFloat(inputs.x0 || '0') - 3);
  const chartXMax = needsInterval ? parseFloat(inputs.b) + 1 : (parseFloat(inputs.x0 || '0') + 3);
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="sm:col-span-2 lg:col-span-3">
            ...
            <Input value={inputs.expr} onFocus={() => setActiveField('expr')} onChange={e => update('expr', e.target.value)} className="math-input" placeholder="x^3 - 2*x - 5" />
          </div>
          {isFixedPoint && (
            <div className="sm:col-span-2 lg:col-span-3">
              <Label className="text-xs mb-1.5 block">g(x) = (función de iteración)</Label>
              <Input value={inputs.gExpr} onFocus={() => setActiveField('gExpr')} onChange={e => update('gExpr', e.target.value)} className="math-input" placeholder="(2*x + 5)^(1/3)" />
            </div>
          )}
          {needsInterval && (
            <>
        ...
        <MathKeyboard onInsert={insertText} onBackspace={backspace} onClear={clearField} onEnter={solve} />
      </div>
